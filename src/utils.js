...
export function populateSelectedCatAndLocationCard(geoCode, category, locationName, table) {
  if (isCatDataFetchedForGeoCode(get(dataByGeography), geoCode, category.code)) {
    ...
    const para2 = `Out of ${categoryData.total} ${categoryData.unit.toLowerCase()} in ${locationStr}, ${
      categoryData.val
    } ${tableNameAsSentenceComponent} is ${categoryNameAsSentenceComponent}.`;
    ...
    const para3 = geoCode != config.eAndWGeoCode ? `This compares to ${ewValue} in England and Wales.` : null;
